...
define(['common/controllers', 'domReady'],
    function (controllers, domReady) {
        ...
        controllers.controller('PinDetailCtrl', function ($scope, PinService, CompanyService, $stateParams, errMap, $state, validation, $q) {
            ...
            $scope.discountArr = [];
            PinService.getDiscountArr().then(function (data) {
                console.log(data);
                $scope.discountArr = data;
                if ($stateParams.activity_id != 0) {
                    PinService.getActivityDetail({activity_id: $stateParams.activity_id}).then(function (data) {
                        ...
                        if($scope.activity.head_discount && $scope.activity.head_discount!=0){
                            var have = false;
                            for(var i in $scope.discountArr){
                                if($scope.activity.head_discount == $scope.discountArr[i].discount){
                                    have = true;
                                    $scope.discount = $scope.discountArr[i];
                                }
                            }
                            if(!have){
                                $scope.discount = {
                                    discount_name:$scope.activity.head_discount/10+"折",
                                    discount:$scope.activity.head_discount
                                }
                                $scope.discountArr.unshift($scope.discount);
                            }
            ...
            $scope.selNorm = function (index) {
                ...
                $scope.activity.norms[index].old_price = $scope.selectNorm[index].company_goods_sale_price/1000;
            ...
            function addSave() {
                ...
                uploadIconPromise.then(function (res) {
                    ...
                    console.log(res);
                    if (res.data.path.indexOf('http') == -1) {
                        $scope.activity.activity_cover = IconIp + res.data.path;
                    }else{
                        $scope.activity.activity_cover = res.data.path;
                    }
                }, function (err) {
